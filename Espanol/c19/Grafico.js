...
class Grafico{
	...
	DatosEntrada(Canvas){
		...
		let MinimoX = parseFloat(document.getElementById("MinimoX").value);
		let MaximoX = parseFloat(document.getElementById("MaximoX").value);
		if (MinimoX >= MaximoX){
			document.getElementById("Observacion").value = "El valor de Mínimo X debe ser menor que Máximo X";
			return;
		}
		
		let MinimoY = parseFloat(document.getElementById("MinimoY").value);
		let MaximoY = parseFloat(document.getElementById("MaximoY").value);
		if (MinimoY >= MaximoY){
			document.getElementById("Observacion").value = "El valor de Mínimo Y debe ser menor que Máximo Y";
			return;
		}		
		...
		for (let Tiempo = Tminimo; Tiempo <= Tmaximo; Tiempo += Tincrementa){
			this.Fotogramas.push(new Fotograma(MinimoX, MinimoY, MaximoX, MaximoY, NumeroLineasGrafico, Tiempo, Evalua));
		}
	...
	Graficar(Lienzo){
		...
		this.Fotogramas[this.FotogramaGrafica].Dibuja(Lienzo);
